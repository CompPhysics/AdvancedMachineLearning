...
So far we have studied models that maximise (a lower bound on) the
\textbf{data log-likelihood}:
...
GANs never evaluate or maximise $\log p_{\btheta}(\bx)$.
Instead, a \textbf{discriminator} $D$ provides the training signal by trying
to tell real from fake.  This adversarial feedback drives the
\textbf{generator} $G$ to match the real distribution — without
ever computing a likelihood.
\end{alertblock}

\medskip
The benefit: sharper samples, no blurry $\ell_2$-averaging.\\
The cost: no likelihood estimate, mode collapse risk, training instability.
\end{frame}
...
\begin{frame}{Binary Cross-Entropy Losses}
  ...
  \textbf{Generator loss} (non-saturating, fake label treated as real):
  \[
    \mathcal{L}_G
    = -\frac{1}{M}\sum_{j=1}^M \log D(G(\bz_j)).
  \]

  \begin{block}{One-sided label smoothing}
    Replace the real target $1$ with $s<1$ (e.g.\ $s=0.9$).
    The optimal discriminator becomes:
    \[
      D^*_s(\bx) = \frac{s\cdot\pr(\bx)}{\pr(\bx) + \pg(\bx)}.
    \]
    This prevents $D$ from becoming overconfident and provides softer
    gradients to $G$ (prevents $\log D(G(\bz))\to-\infty$ when $D$ is perfect).
  \end{block}
\end{frame}
...
\begin{frame}{Wasserstein Distance (WGAN)}
  \begin{columns}[T]
    \column{0.52\textwidth}
      ...
      \medskip
      \textbf{Wasserstein-1 distance} (Earth-mover distance):
      \[
        W(\pr,\pg)
        = \inf_{\gamma\in\Pi(\pr,\pg)}
          \E_{(\bx,\mathbf{y})\sim\gamma}\|\bx-\mathbf{y}\|,
      \]
      where $\Pi(\pr,\pg)$ is all couplings with marginals $\pr$, $\pg$.

    \column{0.45\textwidth}
      \begin{block}{Kantorovich-Rubinstein duality}
        \[
          W(\pr,\pg) = \sup_{\|f\|_L\le 1}
          \Bigl[\E_{\pr}[f(\bx)] - \E_{\pg}[f(\bx)]\Bigr].
        \]
        Supremum over all 1-Lipschitz functions $f$.
      \end{block}
      \begin{alertblock}{Advantage}
        $W$ provides meaningful gradients even when $\pr\cap\pg=\emptyset$.
...
\begin{frame}{WGAN: Gradient Penalty}
  ...
  is sampled uniformly on straight lines between real and fake points.
...
\begin{frame}{Generator Architecture for MNIST}
  ...
  \[
    \bz\in\R^{100}
    \xrightarrow{\text{Linear+BN+LReLU}}
    \R^{256}
    \xrightarrow{\text{Linear+BN+LReLU}}
    \R^{512}
    \xrightarrow{\text{Linear+BN+LReLU}}
...
\begin{frame}{Comparison of GAN Variants}
  \begin{center}
  \renewcommand{\arraystretch}{1.25}
  \begin{tabular}{p{2.2cm}p{3.0cm}p{2.4cm}p{2.8cm}}
    \toprule
    \textbf{Variant} & \textbf{Key change} & \textbf{Loss} & \textbf{Advantage}\\
    \midrule
    Vanilla GAN & FC networks & JS divergence & Simple\\
    DCGAN & Conv.\ networks & JS divergence & Better images\\
    WGAN & Weight clipping & Wasserstein & More stable\\
    WGAN-GP & Gradient penalty & Wasserstein+GP & Avoids clipping\\
    cGAN & Conditioning & JS or W & Controlled gen.\\
    StyleGAN & Style-based & W & State-of-the-art\\
    \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{Key Equations: Summary}
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    Minimax objective
      & $\min_G\max_D\;\E_{\pr}[\log D(\bx)]+\E_{\pz}[\log(1-D(G(\bz)))]$\\[4pt]
    Optimal discriminator
      & $D^*(\bx) = \pr(\bx)/(\pr(\bx)+\pg(\bx))$\\[4pt]
    Nash equilibrium
      & $\pg=\pr$,\quad$D^*\equiv 1/2$\\[4pt]
    Loss at equilibrium
      & $V(G^*,D^*)=-2\log 2$\\[4pt]
    GAN loss = JS div.
      & $V(G,D^*)=2\JS(\pr\|\pg)-2\log 2$\\[4pt]
    Non-sat.\ generator
      & $\mathcal{L}_G=-\E_{\pz}[\log D(G(\bz))]$\\[4pt]
    Discriminator loss
      & $\mathcal{L}_D=-\E_{\pr}[\log D(\bx)]-\E_{\pz}[\log(1-D(G(\bz)))]$\\[4pt]
    Wasserstein distance
      & $W(\pr,\pg)=\sup_{\|f\|_L\le1}(\E_{\pr}[f]-\E_{\pg}[f])$\\[4pt]
    Gradient penalty
      & $\lambda\,\E[({\|\nabla f_w(\hat{\bx})\|_2-1})^2]$\\[4pt]
    MMD
      & $\sqrt{\E_{p\times p}[k]-2\E_{p\times q}[k]+\E_{q\times q}[k]}$\\[4pt]
    FID
...
All three model classes aim to learn $p_{\btheta}(\bx)$, but with
fundamentally different objectives and inductive biases.
...
\midrule
Training objective     & ELBO (lower bound) & ELBO ($T$ terms)    & Minimax / JS-div \\
Likelihood estimate    & $\checkmark$ (ELBO)& $\checkmark$ (ELBO) & $\times$ (none) \\
Latent space           & Compact $\bh\in\R^{d_h}$ & Full-dim $\bx_t$ & Noise $\bz\in\R^k$ \\
Encoder                & Learned $q_{\bphi}(\bh|\bx)$ & Fixed schedule & None \\
Sampling cost          & 1 decoder pass     & $T$ denoiser steps  & 1 generator pass \\
Training stability     & High               & High                & Low--Moderate \\
Sample quality         & Moderate (blurry)  & State of the art    & High (sharp) \\
Mode coverage          & Good               & Excellent           & Partial (collapse)\\
Anomaly detection      & $\checkmark$       & $\checkmark$        & $\times$ \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
...
\begin{block}{VAE}
\textbf{Stable.}
ELBO is a well-defined scalar loss.
Both encoder and decoder trained jointly with standard SGD.
KL term prevents posterior collapse when properly weighted.
The only instability is choosing the right $\beta$ for $\beta$-VAE.
\end{block}

\column{0.33\textwidth}
\begin{block}{Diffusion}
\textbf{Stable.}
MSE on noise prediction is smooth and well-conditioned.
No adversarial dynamics.
Variance of Monte Carlo gradient estimator is low because
$\bx_t$ can be sampled in one step.
The main challenge is choosing the noise schedule.
\end{block}

\column{0.32\textwidth}
\begin{block}{GAN}
\textbf{Unstable by design.}
Two networks update simultaneously in a non-stationary environment.
No guarantee of convergence to Nash equilibrium.
Mode collapse and vanishing gradients require careful mitigation
(WGAN-GP, spectral norm, label smoothing, two-time-scale updates).
\end{block}
\end{columns}

\medskip
\begin{alertblock}{Key insight}
GAN instability stems from the minimax structure:
$V(G,D)$ is a \emph{concave-convex} saddle problem, and simultaneous
gradient ascent-descent does not converge in general even for
bilinear games (cf.\ matching pennies).
\end{alertblock}
\end{frame}
...
\textcolor{green!60!black}{\textbf{Pros}}
\begin{itemize}
  \item \textbf{Principled likelihood bound}: ELBO provides a lower bound on
        $\log p(\bx)$ usable for anomaly detection and model comparison.
  \item \textbf{Compact structured latent space}: low-dimensional $\bh$
        supports interpolation, clustering, disentanglement, property prediction.
  \item \textbf{Fast generation}: single decoder pass at inference.
  \item \textbf{Stable training}: well-defined ELBO objective, no adversary.
  \item \textbf{Flexible posterior}: $\beta$-VAE promotes disentanglement;
        hierarchical VAEs increase expressivity.
  \item \textbf{Scientific utility}: compact latents ideal for
        molecular design, spectral data, physical simulations.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Blurry samples}: Gaussian decoder averages over modes
        (fundamental, not just a capacity issue).
  \item \textbf{Posterior collapse}: powerful decoders can ignore $\bh$;
        KL$\to 0$, latent space useless.
  \item \textbf{ELBO gap}: $\log p(\bx) = \mathrm{ELBO} +
        \DKL(q_{\bphi}\|p(\bh|\bx)) > \mathrm{ELBO}$.
  \item \textbf{Mean-field limitation}: factorised Gaussian misses
        correlated posteriors.
  \item \textbf{Fixed-dim bottleneck}: wrong choice of $d_h$ hurts quality.
\end{itemize}
\end{columns}
\end{frame}
...
\textcolor{green!60!black}{\textbf{Pros}}
\begin{itemize}
  \item \textbf{State-of-the-art sample quality}: sharp, diverse, no blurring.
  \item \textbf{Full distribution coverage}: fixed forward process prevents
        mode collapse; $q(\bx_T|\bx_0)\approx\Norm(\bm{0},\mathbf{I})$ by design.
  \item \textbf{Stable training}: noise-prediction MSE is smooth and
        well-conditioned; no adversarial dynamics.
  \item \textbf{Likelihood bound}: full ELBO converges to $\log p(\bx)$
        as $T\to\infty$.
  \item \textbf{Flexible conditioning}: text, class, depth, audio via
        classifier-free guidance and cross-attention.
  \item \textbf{SDE/score connection}: rich theory via Anderson (1982)
        and Song et al.\ (2021).
\end{itemize}
...
\textcolor{red!60!black}{\textbf{Cons}}
\begin{itemize}
  \item \textbf{Slow sampling}: $T=100$--$1000$ network evaluations
        (DDIM, DPM-Solver reduce to 10--50).
  \item \textbf{No compact latent}: full-dim $\bx_t$ precludes downstream
        clustering or property prediction.
  \item \textbf{Computationally expensive}: large $T$ and high-dim $\bx$
        demand significant GPU memory and training time.
  \item \textbf{Less natural for representation learning}: no encoder
        to extract structured features.
  \item \textbf{Noise schedule sensitivity}: schedule choice affects
        quality; cosine preferred over linear for high-res.
\end{itemize}
\end{columns}
\end{frame}
...
\textcolor{green!60!black}{\textbf{Pros}}
\begin{itemize}
  \item \textbf{Sharpest samples}: adversarial loss penalises blurriness and
        perceptual artefacts that $\ell_2$/ELBO objectives miss.
  \item \textbf{Fastest inference}: single generator pass, no iteration.
  \item \textbf{Flexible loss}: adversarial training works when likelihood is
        intractable or ill-defined.
  \item \textbf{Rich extension ecosystem}: cGAN, DCGAN, WGAN-GP, StyleGAN,
        CycleGAN, pix2pix.
  \item \textbf{Image-to-image translation}: naturally handles paired and
...
\textcolor{red!60!black}{\textbf{Cons}}
\begin{itemize}
  \item \textbf{No likelihood}: cannot evaluate $p_g(\bx)$ for anomaly
        detection or density estimation.
  \item \textbf{Mode collapse}: generator covers only a subset of $p_r$;
        FID and IS may not detect it.
  \item \textbf{Training instability}: non-stationary minimax landscape;
        no convergence guarantee; hyperparameter-sensitive.
  \item \textbf{JS flatness}: $\JS(\pr\|\pg)=\log 2$ when supports
        do not overlap (fixed by WGAN).
  \item \textbf{Discriminator is discarded}: wasted capacity after training.
\end{itemize}
\end{columns}
\end{frame}
...
\begin{center}
\renewcommand{\arraystretch}{1.35}
\begin{tabular}{p{4.8cm}p{2.5cm}p{4.5cm}}
\toprule
\textbf{Use case} & \textbf{Recommended} & \textbf{Reason} \\
\midrule
High-quality image/audio/video synthesis
  & Diffusion & State-of-the-art quality, no collapse \\
Text-to-image (Stable Diffusion)
  & LDM       & VAE compression + diffusion quality \\
Image-to-image translation
  & GAN (pix2pix, CycleGAN) & Adversarial perceptual loss \\
Real-time generation
  & VAE or GAN & Single-pass inference \\
Anomaly detection / density estimation
  & VAE or Diffusion & Likelihood bound available \\
Representation learning / clustering
  & VAE       & Compact structured latent space \\
Scientific simulation (molecules, spectra)
  & VAE       & Low-dim latent enables property prediction \\
Data augmentation (rare events)
  & GAN or Diffusion & Sharp, diverse samples \\
Inpainting / super-resolution
  & Diffusion & Score-based posterior sampling \\
Training with very limited data
  & VAE       & Stable; regularised by KL term \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
...
\begin{enumerate}
  \setlength\itemsep{5pt}
  \item \textbf{Common ELBO structure.}
        Show that the VAE ELBO (1 level) and DDPM ELBO ($T$ levels)
        are both special cases of the MHVAE ELBO.
        Identify the encoder, decoder, and prior in each case.

  \item \textbf{Blurriness from $\ell_2$.}
        Let $p_r(x) = \frac{1}{2}\delta(x-a)+\frac{1}{2}\delta(x-b)$
        (two-point mixture).  Show that the VAE decoder with Gaussian
        likelihood learns $\hat{x}=(a+b)/2$, while a GAN generator can
        learn to produce both $a$ and $b$ with equal probability.

  \item \textbf{GAN vs.\ diffusion convergence.}
        The minimax GAN objective has no gradient-descent convergence
        guarantee. The DDPM ELBO does.  State precisely which property
        of the DDPM ELBO makes it amenable to SGD while the GAN saddle
        problem is not.

  \item \textbf{Wasserstein vs.\ JS.}
        Compute $\JS(\Norm(0,1)\|\Norm(\mu,1))$ and
        $W(\Norm(0,1),\Norm(\mu,1))$ as functions of $\mu$.
        Show that $\JS$ saturates at $\log 2$ for large $|\mu|$ while
        $W=|\mu|$ grows without bound.  Explain why this makes
        WGAN more informative for training.

  \item \textbf{Score matching bridge.}
        The DDPM score $\nabla_{\bx_t}\log q(\bx_t|\bx_0)=-\beps/\sqrt{1-\bar\alpha_t}$.
        Show that the DDPM training objective is equivalent to
        denoising score matching as introduced by Vincent (2011).

  \item \textbf{Latent Diffusion.}
        A VAE compresses $\bx\in\R^{d_x}$ to $\bz\in\R^{d_z}$
        with $d_z=d_x/f^2$.  A DDPM runs in the $\bz$-space with
        $T$ steps.  Express the full LDM training objective as
        a composition of the VAE ELBO and the DDPM ELBO, and
