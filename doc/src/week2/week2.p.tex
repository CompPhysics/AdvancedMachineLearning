...
%% doconce format latex week2.do.txt --latex_title_layout=beamer --latex_table_format=footnotesize --no_mako
%%
...
% !split
\subsection{Overview of second  week, January 26-30, 2026}

\begin{block}{}
\begin{itemize}
\item Mathematics of neural networks

\item Writing own code (bring back to life your NN code if you have one)

\item Discussion of project alternatives
% o \href{{https://youtu.be/9GmKWT2EFwQ}}{Link to video of lecture}
...
% o \href{{https://github.com/CompPhysics/AdvancedMachineLearning/blob/main/doc/HandwrittenNotes/2025/NotesJanuary30.pdf}}{Link to whiteboard notes}
\end{itemize}
...
\item The last layer, normally called \textbf{output} layer has an activation function tailored to the specific problem
...
% !split
\subsection{Why Feed Forward Neural Networks (FFNN)? Classical approximation theorem}
...
This statement is essentially a correct summary of the classical
Universal Approximation Theorem as first proven by Cybenko
(1989) based on activation functions like the \textbf{sigmoid} or \textbf{tanh} functions.

% !split
\subsection{Digression,  more updated variant}
...
on a compact domain in $\mathbb{R}^n$ and any error $\varepsilon>0$, there
...
approximates $f$ within error $\varepsilon$ .

% !split
\subsection{Getting verbose}

The classical theorem’s conditions on the
activation function (namely \textbf{non-constant, bounded, and
...
so-called \textbf{squashing} functions (which are likewise bounded and
...
Later research clarified that the essential
...
% !split
\subsection{GELU Activation Function}

The \textbf{Gaussian Error Linear Unit} (GELU) activation function is defined as
\[
\mathrm{GELU}(x) = x \, \Phi(x),
\]
where $\Phi(x)$ is the cumulative distribution function (CDF) of the standard
normal distribution,
\[
...
% !split
\subsection{Approximation used in practice}
...
\]
This form is computationally efficient and widely used in deep learning
frameworks.
...
\begin{block}{}
\begin{itemize}
\item Smooth and non-linear activation function.

\item Weights inputs by their probability of being positive.

\item Unlike ReLU, negative inputs are suppressed smoothly rather than set to zero.

\item Empirically shown to improve performance in deep networks, especially Transformer-based models.
\end{itemize}
...
% !split
\subsection{Activation Functions Related to GELU}

These activation functions softly gate inputs rather than applying a hard
threshold, improving gradient flow and optimization stability.
\begin{block}{}
\begin{itemize}
\item Swish / SiLU: $\mathrm{Swish}(x) = x\,\sigma(x)$  (logistic-gated variant; widely used in CNNs and modern architectures)

\item Mish: $x\tanh(\ln(1+e^x))$  (smooth, non-monotonic; retains small negative values)
\end{itemize}
...
% !split
\subsection{Related Smooth or Gated Activations}
\begin{block}{}
\begin{itemize}
\item ELU / SELU: exponential smoothing of negative inputs

\item Softplus: smooth approximation to ReLU, common in probabilistic models

\item PReLU: learnable slope for negative inputs (piecewise linear)
\end{itemize}
...
% !split
\subsection{Transformer-Oriented Gated Variants}
\begin{block}{}
\begin{itemize}
\item GLU: explicit learned gating via $\sigma(Wx)$

\item GeGLU: GELU-based gated linear unit

\item SwiGLU: Swish-based gated linear unit (used in modern LLMs)
\end{itemize}

\noindent
\end{block}
% !split
\subsection{Why GELU-like Activations?}

\begin{block}{}
\begin{itemize}
\item Smooth derivatives (important for deep networks and PINNs)

\item Probabilistic or learned gating of information

\item Improved optimization compared to hard-threshold activations (ReLU)
\end{itemize}
...
In general it seems that the GELU/ELU activation functions are better than
the leaky ReLU function (and its variants), which is better than
ReLU. ReLU performs better than $\tanh$ which in turn performs better
than the logistic function.

If runtime performance is an issue, then you may opt for the leaky
ReLU function over the GELU family if you don’t want to tweak yet
another hyperparameter, you may just use the default $\alpha$ of
$0.01$ for the leaky ReLU, and $1$ for say ELU. 

% !split
\subsection{More on activation functions, output layers}

In most cases you can use the ReLU activation function or (preferred for PINNs the GELU family) in the hidden
...
\paragraph{Learning rate methods.}
...
network. Update-change() only has one parameter, the gradient, and returns the change which will be subtracted
...
\paragraph{Usage of the above learning rate schedulers.}
...
% !split
\subsection{Using Pytorch with the full MNIST data set}
...
% !split
\subsection{And a similar example using Tensorflow with Keras}
